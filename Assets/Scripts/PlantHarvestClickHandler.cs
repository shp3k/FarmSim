...
public class PlantHarvestClickHandler : MonoBehaviour
{
    private FarmPlot ownerPlot;
    private Collider2D clickCollider;
    private int lastHarvestAttemptFrame = -1;

    public void Configure(FarmPlot plot)
    {
        ownerPlot = plot;
    ...
    private void TryHarvest()
    {
        if (ownerPlot == null)
        {
            ownerPlot = GetComponentInParent<FarmPlot>();
        }
        ...
        if (lastHarvestAttemptFrame == Time.frameCount)
        {
            return;
        }

        lastHarvestAttemptFrame = Time.frameCount;
        ownerPlot.HarvestForProfit(out _, out _);
    }
